...
static int darwin_reset_device(struct libusb_device_handle *dev_handle) {
  ...
  while (dpriv->in_reenumerate) {
    struct timespec delay = {.tv_sec = 0, .tv_nsec = 20};
...
static int darwin_handle_transfer_completion (struct usbi_transfer *itransfer) {
    /**
     Add null pointer protection, avoid itransfer null crash.
     (eg.RCVR-7093 - [Once]Host crash when plug cameras)
     */
    if (itransfer == nil) {
        return LIBUSB_ERROR_IO;
    }
    if (itransfer->num_iso_packets < 0) {
        return LIBUSB_ERROR_IO;
    }
    
    if (itransfer->num_iso_packets > 10) {
        return LIBUSB_ERROR_IO;
    }
